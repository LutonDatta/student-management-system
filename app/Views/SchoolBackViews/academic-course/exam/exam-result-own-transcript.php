...
<div class="row d-print-none">
    <div class="col-12 text-center">
        <div class="form-group">
            <label class="col-form-label" for="select_scm"><?=myLang('Select class here to get marksheet','নম্বরপত্র দেখার জন্য এখান থেকে ক্লাশ নির্বাচন করুন');?></label>
            <select class="form-control form-control-lg" id="select_scm_on_change">
                <option><?=myLang('Select class from here','নম্বরপত্র দেখার জন্য ক্লাশ নির্বাচন করুন');?></option>
                <?php foreach($studentAllSCMList as $opt): ?>
                    <option <?=(intval($studentSCM->scm_id) === intval($opt->scm_id)) ? 'selected="selected"' : '';?> value="<?=esc($opt->scm_id);?>">
                        <?php $clsObjF = service('ClassesAndSemestersModel')->withDeleted()->get_single_class_with_parent_label(intval($opt->scm_class_id)); ?>
                        <?= is_object($clsObjF) ? esc($clsObjF->title) : '';?>
                        - <?= esc( get_student_class_status()[$opt->scm_status] );?>
                        - Session: <?= esc( $opt->scm_session_year );?>
                        - Roll: 
                        - SCM ID: <?=esc($opt->scm_id);?>
                    </option>
                <?php endforeach; ?>
            </select>
            <script {csp-script-nonce}>
                document.addEventListener("DOMContentLoaded", function(){        
                    $( "#select_scm_on_change" ).change(function(){
                        window.location.href = "<?=base_url("admin/academic/exam/results/view/own?marksheet_own_view_scm_id=");?>" + $(this).val();
                    });
                });
            </script>
        </div>
            ...
            <div class="row">
                <div class="col-12 text-center mb-3 pb-3 border-bottom">
                    ...
                    <div class="h6 mt-0 mb-0 pt-0 li">
                        <?=esc(getSchool()->sch_address);?>
                    </div>
                    <div class="h6 mt-0 mb-0 pt-0 li">Contact: <?=esc(get_option('instNameEn'));?></div>
                </div>
                ...
                <?php else: ?>
                    <div class="col-12">
                        <div class="text-center border m-2 label-info">
                            <?=myLang('No exam result found for you, please talk to your teacher.','আপনার কোন পরীক্ষার ফলাফল পাওয়া যায়নি। অনুগ্রহ করে আপনার শিক্ষকের সাথে যোগাযোগ করুন।');?>
