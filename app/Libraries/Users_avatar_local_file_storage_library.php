...
class Users_avatar_local_file_storage_library{
    /**
     * @var array
     */
    protected $options;
    ...
    public function __construct( object $request, object $response ){
        ...
        $this->options = array(
            ...
            'upload_url'        => school_url('arf/p/'), // Attachment ID will be added here to see the image
    ...
    protected function loadAction(){
        ...
        $user = service('UserStudentsModel')->select('u_thumbnail_id')->find(service('AuthLibrary')->getLoggedInUserID());
        if( !is_object($user)) { return $this->generateResponse($defaultImage); }
        $att = service('AttachmentsThumbModel')->find(intval($user->u_thumbnail_id));
        if( ! is_object($att) ) {return $this->generateResponse($defaultImage); }
        
        $filepath = service('AttachmentsThumbModel')->get_attachment_full_path( $att );
        ...
        if(file_exists($filepath) AND ! is_dir($filepath)){
            ...
            $image = new \stdClass();
            $image->name = $att->atp_id . '.' . $att->atp_ext;
            $image->type = $att->atp_ext;
            $image->url  = school_url("arf/p/{$att->atp_id}.{$att->atp_ext}");
    ...
    protected function previewAction(){
        ...
        $filepath = service('AttachmentsThumbModel')->get_attachment_full_path( $att ); // Sourse file
        if( ! (file_exists($filepath) AND !is_dir($filepath))){
            return $this->generateResponse(['error' => "Error in previewAction - Local File not exists."]);
        }

        return $this->response
                ->setHeader('Content-Length', filesize($filepath))
                ->setHeader('Content-type', $att->atp_type) // Should be like: image/jpeg
    ...
    protected function handleFileUpload($uploaded_file, $name, $size, $error){
        ...
        $data = [
            'atp_ext'        => $image->type,
            'atp_type'       => $fileObj->getMimeType(),
            'atp_size'       => ceil( $image->size / 1024 ),  // 250.880 -> 250
        ];
            
        $attLib = service('AttachmentsThumbModel');
        $att    = service('AttachmentsThumbModel')->find(intval($user->u_thumbnail_id));
        if( ! is_object( $att ) ){
            ...
            $this->upload_attachment_path   = $attLib->get_attachment_path($att->atp_id) . DIRECTORY_SEPARATOR;
            $this->upload_attachment_id     = $att->atp_id;
        ...
        $image->url = school_url("arf/p/{$this->upload_attachment_id}.{$image->type}");
        ...
        try{
            $imgMv = $this->request->getFile('file')->move( 
            ...
            if( ! empty($overwritePreviousAvatar)){
                ...
                service('AttachmentsThumbModel')->update($this->upload_attachment_id, $data);
            }
            // File uploaded now crop automatically
            $filePathImg = WRITEPATH . $this->upload_attachment_path . $image->name;
            
            /* Image processing throw out of memory error. To solve this, reduce px of image before cropping. */
            if( intval($image->width) > 2000 AND intval($image->height) > 2000 ){
                $remainderW = floor(intval($image->width) / 300);
                $remainderH = floor(intval($image->height) / 300);
                $reducedW = ceil(intval($image->width) / $remainderW);
                $reducedH = ceil(intval($image->height) / $remainderH);
                /* Resize to lowest px but not less then 300x300px, otherwise we might get out of memory error if we trigger fit() */
                \Config\Services::image()->withFile($filePathImg)->reorient()->resize($reducedW, $reducedH, true)->save($filePathImg);
                /* Now crop and resize to 300x300x again to get desired result. It will take less memory as image diemnstion is reduced */
                \Config\Services::image()->withFile($filePathImg)->reorient()->fit(300, 300, 'center')->save($filePathImg);
            }else{
                // Fix mobile orientation + (Crop + Resize = 2 actions)
                \Config\Services::image()->withFile($filePathImg)->reorient()->fit(300, 300, 'center')->save($filePathImg);
            }
        }catch(Exception $e){
        ...
        if( ! is_object($att) ){ 
            ...
            service('UserStudentsModel')->update(service('AuthLibrary')->getLoggedInUserID(),['u_thumbnail_id'=>$this->upload_attachment_id]);
        }
    ...
    protected function cropAction(){
        ...
        $image->name    = $att->atp_id . '.' . $att->atp_ext;
        $image->type    = $att->atp_ext;
        $image->url     = school_url('arf/p/' . $att->atp_id  );
        ...
        try{
            $process = \Config\Services::image()->withFile($filepath)
                ...
                ->fit(300, 300, 'center')    // Crop + Resize = 2 actions
                ->save($filepath);
        ...
        service('AttachmentsThumbModel')->limit(1)->update($att->atp_id, [ 'atp_size' => ceil( $fileObj->getSize() / 1024 ) ]);
