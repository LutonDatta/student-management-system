...
class Users_sign_upload_by_teacher_library{
    /**
     * @var array
     */
    protected $options;
...
//        $defaultImage->height   = 300;
        ...
        $user = service('UserStudentsModel')->select('u_sign_photo_id')->find(service('request')->getGet('load_sign_of'));
    ...
    protected function handleFileUpload($uploaded_file, $name, $size, $error){
        ...
        if( ! is_object($att) ){ 
            ...
            service('UserStudentsModel')->update(service('request')->getGet('load_sign_of'),['u_sign_photo_id'=>$this->upload_attachment_id]);
        }
