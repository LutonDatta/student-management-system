...
//        $defaultImage->height   = 300;
        ...
        $user = service('StudentsModel')->select('u_sign_photo_id')->find(service('request')->getGet('load_sign_of'));
    ...
    protected function handleFileUpload($uploaded_file, $name, $size, $error){
        ...
        if( ! is_object($att) ){ 
            ...
            service('StudentsModel')->update(service('request')->getGet('load_sign_of'),['u_sign_photo_id'=>$this->upload_attachment_id]);
        }
